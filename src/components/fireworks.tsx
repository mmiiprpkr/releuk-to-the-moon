...
export default function Fireworks() {
  ...
  const createFirework = (canvas: HTMLCanvasElement, x?: number): Firework => {
    ...
    const specialEffect =
      Math.random() < 0.5 ? getRandomSpecialEffect() : undefined;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      switch (firework.pattern) {
        ...
        case "star":
          ...
          const starRadius =
            1 +
            (0.5 * Math.floor(i / starPoints)) / (particleCount / starPoints);
          angle = starAngle;
  ...
  const createSplitExplosion = (particle: Particle, isSecondStage = false) => {
    const particles: Particle[] = [];
    const particleCount = isSecondStage
          ...
          for (let i = 0; i < batchSize; i++) {
            setTimeout(() => {
              if (fireworksRef.current.length < MAX_FIREWORKS) {
                ...
                const launchX =
                  centerX + (Math.random() - 0.5) * (canvas.width * 0.8);
          ...
          firework.particles = firework.particles.filter((particle) => {
            ...
            if (particle.wiggle && particle.wiggle > 0) {
              const wiggleAmount =
                particle.wiggle *
                Math.sin(
                  particle.life * particle.wiggleSpeed! + particle.wiggleOffset!
                );
            ...
            if (particle.alpha > 0) {
              ...
              if (glitter) {
                ...
                ctx.fillStyle = `hsla(${particle.hue}, 80%, 90%, ${
                  currentAlpha * 0.3
                })`;
                ctx.fill();
              }
